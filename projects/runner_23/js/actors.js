    ...
    update() {
        ...
        const checkSide = side => {
            const item = world.grid[world.findIndex({
                x: Math.round(this.pos.x) + side,
                y: Math.round(this.pos.y)
            })]

            if (item instanceof Wall && this.direction == side)
                this.direction *= -1
        }
            ...
            if (e instanceof Lift) {
                ...
                if (margin.x || margin.y) {
                    ...
                    if (collide.squish && Math.sign(margin.y) != Math.sign(collide.squish) &&
                        Math.abs(margin.y) + Math.abs(collide.squish) > 0.3)
                            this.destroy()
                }
    ...
    update() {
        ...
        if (!this.dead) {
            ...
            if (!this.time) {
                ...
                else if (action <= 6) this.direction = 0

                else if (action <= 10 && !this.direction && !this.inWater)
